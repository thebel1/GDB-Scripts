...
# - Implement calculating concrete values.
# - Implement AST representation of execution paths.

# References:
# - https://triton.quarkslab.com/blog/What-kind-of-semantics-information-Triton-can-provide/
...
class GDB_BackSlice(gdb.Command):
    ...
    def invoke(self, argstr, from_tty):
        ...
        target_reg = None
        target_reg_name = ''
        if args.target_reg in x64RegToTritonSymbolMapping.keys():
        ...
        while rip in bytes_dict:
            ...
            if rip == target_addr:
                symregs = ctx.getSymbolicRegisters()
                try:
                    target_symreg = symregs[target_reg]
                    ...
                    for instr in exec_path:
                        print(instr)
                    if params['flags']['print_symbolic']:
                        print('\nsymbolic:')
                        self.printSymbolicRegisters(ctx, symregs)
                    return
                sliced = ctx.sliceExpressions(target_symreg)
    ...
    def validateParams(self, params):
        ...
        if 'regs' in params.keys():
            if type(params['regs']) != dict:
                error('params["regs"] must be a dictionary')
                raise TypeError
            for k, v in params['regs'].items():
                if k not in x64RegToTritonSymbolMapping:
                    error(f'invalid register in params["regs"]: {k}')
                    raise TypeError
                if type(v) not in [int, str]:
                    error(('values in params["regs"] must be of type string'
                           f' or int; key: {k}, value: {v}; type: {type(v)}'))
                    raise TypeError

        if 'mem' in params.keys():
            if type(params['mem']) != dict:
                error('params["mem"] must be a dictionary')
                raise TypeError
            for k, v in params['mem'].items():
                if type(k) != str:
                    error('keys in params["mem"] must be of type string')
                    raise TypeError
                if type(v) not in [str, int]:
                    error(('values in params["mem"] must be of type string'
                           f' or int; key: {k}, value: {v}; type: {type(v)}'))
                    raise TypeError

        if 'flags' in params.keys():
            if type(params['flags']) != dict:
                flags_type = type(params['flags']).__name__
                error(('params["flags"] must be a dictionary;'
                       f' type: {flags_type}'))
                raise TypeError
            for k, v in params['flags'].items():
                if k not in backSliceParams['flags'].keys():
                    error(f'invalid flag in params["flags"]: {k}')
                    raise TypeError
                if type(v) != bool:
                    v_type = type(v).__name__
                    error(('flags in params["flags"] must be of type boolean;'
                           f' key: {k}, value: {v}; type: {v_type}'))
                    raise TypeError
        ...
        if 'regs' in new_params.keys():
            for k, v in new_params['regs'].items():
                try:
                    ...
                    error(('unable to parse register expression in'
                           f' params["regs"]; key: {k}, value: {v}'))
                    raise TypeError
        
        if 'mem' in new_params.keys():
            for k, v in new_params['mem'].items():
                try:
                    ...
                    error(('unable to parse key-value pair in params["mem"]:'
                           f' key: {k}, value: {v}'))
                    raise TypeError
        
        if 'flags' in new_params.keys():
            for k, v in new_params['flags'].items():
                params['flags'][k] = v
    ...
    def printSymbolicRegisters(self, ctx, symregs):
        rev_mapping = {v: k for k, v in x64RegToTritonSymbolMapping.items()}
        for reg, sym in symregs.items():
            name = None
            origin = str(sym.getOrigin())
            cmt = sym.getComment()
            print(f'{origin:<32} {cmt}')
